...
void TSTARKPhysics::BuildPhysicalEvent() {
  ...
  std::map<int, double> csiE_map;               // (group number, E)
  std::map<int, int> firstHitIdx_map;           // (group number, first hit index)
  ////////////////////////////////////////////////////////////
  ...
  for (Int_t i = 0; i < m_EventData->GetMult(); i++) {
    type[i] = m_EventData->GetType(i);  // 0: X6, 1: BB10, 2: QQQ5, 3: CsI
    ...
    if (type[i] == 0) {  // for X6
      sPosArr.push_back(m_StripPos[detN[i] - 1][fStrN[i] - 1][bStrN[i] - 1]);
      ////////////////////////////////////////
      ...
      ////////////////////////////////////////
    } else if (type[i] == 1 || type[i] == 2) {  // for BB10 & QQQ5
      sPosArr.push_back(m_StripPos[detN[i] - 1][fStrN[i] - 1][bStrN[i] - 1]);
      hPosArr.push_back(
          m_StripPos[detN[i] - 1][fStrN[i] - 1][bStrN[i] - 1]);  // detector and strip number from 1 (not 0)
    }  // nothing to do for CsI

    ////////////////////////////////////////////////////////////
    // for dE-E analysis
    //
    // Note: detN is from 1 (not 0)
    int group = m_Group[(detN[i] - 1) % 100];  // also considering the CsI case (detID + 300 & 400)
    if (group == 0) continue;

    if (type[i] < 3) {                                         // for X6, BB10, QQQ5
      dE_map[group][m_GroupLayer[detN[i] - 1] - 1] = sumE[i];  // layer number from 1 (not 0)
      E_map[group] += sumE[i];
      if (m_GroupLayer[detN[i] - 1] == 1) {
        firstHitIdx_map[group] = i;
      }
    } else if (type[i] == 3) {  // for CsI
      csiE_map[group] += sumE[i];
    }
    ////////////////////////////////////////////////////////////
  }
  ////////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < nGroup; i++) {
    ...
    if (csiE_map.find(groupN[i]) != csiE_map.end()) {
      groupEwithCsI[i] = groupE[i] + csiE_map[groupN[i]];
    } else {
      groupEwithCsI[i] = groupE[i];
    }
    groupFirstHitIdx[i] = firstHitIdx_map[groupN[i]];
  }
...
void TSTARKPhysics::InitializeRootOutput() {
  ...
  outputTree->Branch("groupEwithCsI", groupEwithCsI, "groupEwithCsI[nGroup]/D");
  outputTree->Branch("groupdE", groupdE, "groupdE[20][20]/D");  // should use fixed-length array for 2-dim array
  outputTree->Branch("groupFirstHitIdx", groupFirstHitIdx, "groupFirstHitIdx[nGroup]/I");
}
