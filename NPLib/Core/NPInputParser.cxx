...
std::string NPL::StripSpaces(std::string line) {
  ...
  while (*line.begin() == ' ') line = line.substr(1, line.length());

  // Remove trailing spaces
  if (line.length() > 0)
    while (*line.rbegin() == ' ') line = line.substr(0, line.length() - 1);
...
std::string NPL::InputBlock::ExtractToken(std::string line, std::string separator) {
  // Find the separator
  if (separator == "") separator = NPL::token_separator;
...
bool NPL::InputBlock::HasTokenList(std::vector<std::string> Token) {
  if (m_Token.size() == 0) return false;

  bool res = true;
  for (unsigned int i = 0; i < Token.size(); i++) res = res && HasToken(Token[i]);
...
std::vector<std::string> NPL::InputBlock::GetMissingToken(std::vector<std::string> Token) {
  ...
  for (unsigned int i = 0; i < Token.size(); i++) {
    if (!HasToken(Token[i])) missing.push_back(Token[i]);
  }
  return missing;
}
...
std::string NPL::InputBlock::GetValue(std::string Token) {
  for (unsigned int i = 0; i < m_Token.size(); i++) {
    if (m_Token[i] == ToLower(Token)) return m_Value[i];
  }
  return "";
}
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
bool NPL::InputBlock::GetBool(std::string Token) {
  int verbose = 1;  // NPOptionManager::getInstance()->GetVerboseLevel();
  ...
  if (value == "true" || value == "1")
    ...
    std::cout << "WARNING: Invalid boolean value for token " << Token << std::endl;
    val = false;
  }

  if (verbose) printf(" %s: %s\n", Token.c_str(), val ? "true" : "false");
  return val;
}
////////////////////////////////////////////////////////////////////////////////
...
std::vector<std::string> NPL::InputBlock::GetVectorString(std::string Token, std::string separator) {
  ...
  std::vector<std::string> val;
  std::string buffer;
  while (iss >> buffer) val.push_back(buffer);

  if (verbose) {
    printf(" %s: ", Token.c_str());
    // std::cout << " " << Token << ": ";
    for (unsigned int i = 0; i < val.size(); i++) printf("%s ", val[i].c_str());
    // std::cout << val[i] << " " ;
...
std::vector<double> NPL::InputBlock::GetVectorDouble(std::string Token, std::string default_unit) {
  ...
  if (unit == "") {
    if (default_unit != "void") std::cout << "WARNING: Using default units for token " << Token << std::endl;
    for (unsigned int i = 0; i < val.size(); i++) val[i] = ApplyUnit(val[i], default_unit);
  } else {
    for (unsigned int i = 0; i < val.size(); i++) val[i] = ApplyUnit(val[i], unit);
  }

  if (verbose) {
    printf(" %s (%s): ", Token.c_str(), default_unit.c_str());
    // std::cout << " " << Token << " (" << default_unit << "): ";
    for (unsigned int i = 0; i < val.size(); i++) printf("%f ", val[i] / ApplyUnit(1, default_unit));
...
std::vector<int> NPL::InputBlock::GetVectorInt(std::string Token) {
  ...
  if (verbose) {
    printf(" %s: ", Token.c_str());
    // std::cout << " " << Token << ": ";
    for (unsigned int i = 0; i < val.size(); i++) printf(" %d: ", val[i]);
    //  std::cout << val[i] << " " ;
    std::cout << std::endl;
  }
...
void NPL::InputParser::Dump() {
  for (unsigned int i = 0; i < m_Block.size(); i++) m_Block[i]->Dump();
}
////////////////////////////////////////////////////////////////////////////////
...
std::vector<NPL::InputBlock*> NPL::InputParser::GetAllBlocksWithTokenAndValue(std::string Token, std::string Value) {
  ...
  for (unsigned int i = 0; i < m_Block.size(); i++) {
    if (m_Block[i]->GetMainToken() == Token) {
      if (m_Block[i]->GetMainValue() == Value) res.push_back(m_Block[i]);
    }
  }
...
void NPL::InputParser::TreatAliases() {
  ...
  for (unsigned int i = 0; i < alias.size(); i++) {
    ...
    if (verbose) std::cout << "Using Alias : @" << alias[i]->GetMainValue() << std::endl;
    ...
    if (value[0].find("(") != std::string::npos) value = alias[i]->GetVectorList("Value");
    ...
    for (unsigned int b = 0; b < m_Block.size(); b++) {
      ...
      if (action == "Replace") {
        ...
        for (unsigned int v = 0; v < size; v++) {
          if (m_Block[b]->GetValue(v).find(name) != std::string::npos) check = true;
        }
        if (check) {
...
void NPL::InputParser::ReadFile(std::string filename, bool ExitOnError) {
  ...
  while (!file.eof()) {
    ...
    if (IsNotComment(line) && GetLevel(line) == 0) {
      if (block) m_Block.push_back(block);
...
void NPL::InputParser::Clear() {
  for (unsigned int i = 0; m_Block.size(); i++) delete m_Block[i];
...
double NPL::ApplyUnit(double value, std::string unit) {
  ...
  else if (unit == "torr" || unit == "Torr") {
    return value * NPUNITS::torr;
  }
