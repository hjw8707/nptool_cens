...
void NPOptionManager::ReadProjectConfigFile() {
    ...
    if (ProjectFile.is_open()) {
        ...
        for (unsigned int i = 0; i < size; i++) {
            if (blocks[i]->HasToken("AnalysisOutput")) m_AnalysisOutputPath = blocks[i]->GetString("AnalysisOutput");
            ...
            if (blocks[i]->HasToken("EnergyLoss")) m_EnergyLossPath = blocks[i]->GetString("EnergyLoss");

            if (blocks[i]->HasToken("Cuts")) m_CutsPath = blocks[i]->GetString("Cuts");
        }
    }
...
void NPOptionManager::ReadTheInputArgument(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (argument == "-DC" && argc >= i + 1)
            fDoCalibrationFileName = argv[++i];

        else if (argument == "-EF" && argc >= i + 1)
            fEfficiencyFileName = argv[++i];

        else if (argument == "-TC")
            fTestCalibration = true;
...
void NPOptionManager::DisplayVersion() {
    if (fVerboseLevel > 0) {
        ...
        std::cout << line << std::endl;
        std::cout << " NPLib version: nplib-" << NPL::version_major << "-" << NPL::version_minor << "-"
                  << NPL::version_deta << std::endl;
...
void NPOptionManager::CheckG4Macro() {
    if (IsDefault("G4MacroPath")) return;
...
void NPOptionManager::SendErrorAndExit(const char* type) const {
    ...
    if (stype == "EventGenerator") {
        ...
        std::cout << "**********************************       Error       **********************************"
                  << std::endl;
        std::cout << "* No event generator file found in $NPTool/Inputs/EventGenerator or local directories *"
                  << std::endl;
        std::cout << "***************************************************************************************"
                  << std::endl;
        std::cout << std::endl;
