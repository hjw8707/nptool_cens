...
#include "TKey.h"
#include "TROOT.h"
void ProgressDisplay(clock_t&, clock_t&, unsigned long&, unsigned long&, unsigned long&, double&, unsigned long&, int&,
                     int&);
void ProgressDisplay(struct timeval& begin, struct timeval& end, unsigned long& treated, unsigned long& inter,
                     unsigned long& total, double& mean_rate, unsigned long& displayed, int& current_tree,
                     int& total_tree);
////////////////////////////////////////////////////////////////////////////////
int main(int argc, char** argv) {
    ...
    // Case of a Physics tree produced
        ...
        if (myOptionManager->IsDefault("OutputFileName")) OutputfileName = "PhysicsTree";
    }

    // Case of Result tree produced
    else {
        TreeName = "ResultTree";
        if (myOptionManager->IsDefault("OutputFileName")) OutputfileName = "ResultTree";
    }
    ...
    if (nentries + first_entry > Chain->GetEntries()) {
        nentries = first_entry + Chain->GetEntries();
    }
    ...
    if (UserAnalysis == NULL) {
        if (!IsPhysics) {
            for (unsigned long i = first_entry; i < nentries + first_entry; i++) {
                ...
                ProgressDisplay(tv_begin, tv_end, treated, inter, nentries, mean_rate, displayed, current_tree,
                                total_tree);
            ...
            std::cout << "\033[1;31m ERROR: You are requesting to rebuild a Physics Tree without any User Analysis, "
                         "nothing to be done\033[0m"
                      << std::endl;
            // Quit without error
            exit(0);
        }
    ...
    if (myOptionManager->GetGenerateHistoOption()) myDetector->WriteSpectra();
...
void ProgressDisplay(struct timeval& begin, struct timeval& end, unsigned long& treated, unsigned long& inter,
    ...
    if (elapsed > 1 || treated >= total) {
        ...
        if (treated != total)
            printf(
                "\r \033[1;31m %s Progress: \033[1;36m%.1f%% \033[1;31m| Rate: %.1fk evt/s | Remain: %s | Tree: %d/%d "
                "%s   \033[0m         ",
                star, percent, mean_rate / 1000., timer, current_tree, total_tree, star);

        else {
            printf(
                "\r \033[1;32m %s Progress: %.1f%% | Rate: %.1fk evt/s | Remain: %s | Tree: %d/%d %s   \033[0m        "
                " ",
                star, percent, mean_rate / 1000., timer, current_tree, total_tree, star);
        }
        fflush(stdout);
...
////////////////////////////////////////////////////////////////////////////////
void ProgressDisplay(clock_t& begin, clock_t& end, unsigned long& treated, unsigned long& inter, unsigned long& total,
                     double& mean_rate, unsigned long& displayed, int& current_tree, int& total_tree) {
    end = clock();
