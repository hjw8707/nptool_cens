...
void NPL::CalibrationSource::ComputeSourceSignal() {
  ...
  if (m_FitFunctionType == 0) { // 3-alpha source golobal (8 gaussians)
    ...
    m_SourceSignal->SetParLimits(9, 0.95 * m_Energies[1][2] / m_Energies[1][0],
                                 1.05 * m_Energies[1][2] / m_Energies[1][0]);
    ...
    for (unsigned int i = 0; i < m_Energies.size(); i++) {
      for (unsigned int j = 0; j < m_Energies[i].size(); j++) {
        contribution += Form("(%f)*[%i]*exp(-(x-[%i]*%f)*(x-[%i]*%f)/(2*[%i]*[%i]))",
                             m_BranchingRatio[i][j] / m_BranchingRatio[i][0], arg_number, arg_number + 1,
                             m_Energies[i][j] / m_Energies[i][0], arg_number + 1, m_Energies[i][j] / m_Energies[i][0],
                             arg_number + 2, arg_number + 2);
        if (j != m_Energies[i].size() - 1)
          contribution += "+";
      }

      arg_number += 3;
      if (i != m_Energies.size() - 1)
        contribution += "+";
    }
...
double AlphaSourceGaus(double* x, double* p) {
  ...
  if (p[2] == 0) {
    cout << "sigma=0 in AlphaTripletGaus" << endl;
    return fitval;
  }
...
double AlphaTripletWithTwoTail(double* x, double* p) {
  ...
  double fitval = p[0] / 2 * p[4] *
                      ((1 - p[11]) / p[3] * TMath::Exp(arg1) * TMath::Erfc(arg2) +
                       p[11] / p[10] * TMath::Exp(arg1b) * TMath::Erfc(arg2b)) +
                  p[0] / 2 * p[5] *
                      ((1 - p[11]) / p[3] * TMath::Exp(arg3) * TMath::Erfc(arg4) +
                       p[11] / p[10] * TMath::Exp(arg3b) * TMath::Erfc(arg4b)) +
                  p[0] / 2 * p[6] *
                      ((1 - p[11]) / p[3] * TMath::Exp(arg5) * TMath::Erfc(arg6) +
...
////////////////////////////////////////////////////////////////////////////////
void NPL::CalibrationSource::AddParticleContribution(vector<double> Energies, vector<double> ErrEnergies,
