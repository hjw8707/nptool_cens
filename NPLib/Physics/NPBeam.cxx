...
Beam::Beam() {
    ...
    int offset = 0;
    while (gDirectory->FindObjectAny(Form("EnergyHist_%i", offset)) != 0) ++offset;
...
void Beam::ReadConfigurationFile(NPL::InputParser parser) {
    ...
    vector<string> emmitA = {"SigmaThetaX", "SigmaPhiY", "SigmaX", "SigmaY",
                             "MeanThetaX",  "MeanPhiY",  "MeanX",  "MeanY"};
    ...
    for (unsigned int i = 0; i < blocks.size(); i++) {
        if (blocks[i]->HasTokenList(token)) {
            ...
            if (blocks[i]->HasToken("UseNearestEnergyLevel"))
                fUseNearestEnergyLevel = blocks[i]->GetInt("UseNearestEnergyLevel");

            if (blocks[i]->HasToken("ZEmission")) fZEmission = blocks[i]->GetDouble("ZEmission", "mm");

            if (blocks[i]->HasToken("ZProfile")) fZProfile = blocks[i]->GetDouble("ZProfile", "mm");
    ...
    if (NPOptionManager::getInstance()->GetVerboseLevel()) cout << "\033[0m";
}
...
void Beam::GenerateRandomEvent(double& E, double& X, double& Y, double& Z, double& ThetaX, double& PhiY) {
    ...
    if (fSigmaEnergy != -1) {
        E = -1;
        while (E < 0) E = gRandom->Gaus(fEnergy, fSigmaEnergy);
    }
    // User Profile
    else
