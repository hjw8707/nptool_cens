...
def PrintBlock(x, y, z, a, b, c):
    ...
    print(" ShieldThickness= 65 um")  # 65-um ESR (mylar instead)
...
#
def BottomWall(n_xlayers = 10, distance = 105.):
    global n_cacao
    (a, b, c) = (0, 90, 0)
    y = -abs(distance)
...
#
def SideWall(n_ylayers = 14, distance = 105):
    global n_cacao
    (a, b, c) = (0, 90, 90)
    x = abs(distance)
    for i in range(n_zlayer):
...
########################################################
# Top Wall
def TopWall(n_xlayers = 10, distance = 105):
    global n_cacao
    (a, b, c) = (0, -90, 0)
    y = abs(distance)
    for i in [0, 1, 8, 9, 10, 11, 12, 13]:
        z = z_start + 16.*i
        if i == 8 or i == 9:
            for iblock in [0, 1, n_xlayers-2, n_xlayers-1]: # n layers
                x = (iblock - (n_xlayers-1)/2.) * 16.
                PrintBlock(x,y,z,a,b,c)
                n_cacao = n_cacao + 1
            continue
...
if __name__=="__main__":
    ...
    if len(sys.argv) > 1:
        ...
        dist = 105
    BottomWall(distance=dist)
    print("%% Bottom total = %d" % n_cacao)
    SideWall(distance=dist)
    print("%% Bottom + Side total = %d" % n_cacao)
    TopWall(distance=dist)
