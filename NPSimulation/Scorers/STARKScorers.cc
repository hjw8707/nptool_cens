 ...
 * PAD detector (any STARK Detector)                                       *
 *****************************************************************************/
#include "STARKScorers.hh"
#include <G4Types.hh>

using namespace STARKSCORERS;
...
PS_STARK_CsI::PS_STARK_CsI(G4String name, G4int Level, G4int depth) : G4VPrimitiveScorer(name, depth), HCID(-1) {
  m_Level = Level;
}
...
G4bool PS_STARK_CsI::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  ...
  G4double* EnergyAndTime = new G4double[4];

  auto copyNo = aStep->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(m_Level);
  m_detectorNumber = copyNo / 100;
  G4int csiNumber = copyNo % 100;
  EnergyAndTime[0] = m_detectorNumber;
  EnergyAndTime[1] = csiNumber;
  EnergyAndTime[2] = aStep->GetTotalEnergyDeposit();
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_X6::PS_STARK_X6(G4String name, G4int Level, G4double stripPlaneX, G4double stripPlaneY, G4int nFrontStrip,
                         G4int nBackStrip, G4int depth)
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_X6::~PS_STARK_X6() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4bool PS_STARK_X6::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  // contain E_DW, E_UP, E_Bot, Time, DetNbr, and StripWidth
  G4double* EnergyAndTime = new G4double[12];

  EnergyAndTime[3] = aStep->GetPreStepPoint()->GetGlobalTime();

  G4int copyNo = aStep->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber(m_Level);
  m_detectorNumber = copyNo / 100;
  ...
  m_Position =
      aStep->GetPreStepPoint()->GetTouchableHandle()->GetHistory()->GetTopTransform().TransformPoint(m_Position);

  m_frontStripNumber =
      (int)((-m_Position.x() + m_stripPlaneX / 2.) / m_frontStripPitch) + 1; // reverse direction for X (from 1)
  m_backStripNumber =
      (int)((m_Position.y() + m_stripPlaneY / 2.) / m_backStripPitch) + 1; // forward direction for Y (from 1)

  ////////////////////////////////////////////////////////////
  // Not active area: wrong strip numbers -> not hit
  if (m_frontStripNumber < 1 || m_frontStripNumber > m_nFrontStrip || m_backStripNumber < 1 ||
      m_backStripNumber > m_nBackStrip)
    return false;
  ////////////////////////////////////////////////////////////
  ...
  // Downstream Energy
  EnergyAndTime[0] =
      aStep->GetTotalEnergyDeposit() * (1 + P) * 0.5; // downsteam collects MORE energy when P is positive
  // Upstream Energy
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_X6::Initialize(G4HCofThisEvent* HCE) {
  EvtMap = new NPS::HitsMap<G4double*>(GetMultiFunctionalDetector()->GetName(), GetName());
  if (HCID < 0)
    HCID = GetCollectionID(0);
  HCE->AddHitsCollection(HCID, (G4VHitsCollection*)EvtMap);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_X6::EndOfEvent(G4HCofThisEvent*) {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_X6::clear() {
  std::map<G4int, G4double**>::iterator MapIterator;
  for (MapIterator = EvtMap->GetMap()->begin(); MapIterator != EvtMap->GetMap()->end(); MapIterator++) {
    delete *(MapIterator->second);
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_X6::DrawAll() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_X6::PrintAll() {
  G4cout << " MultiFunctionalDet  " << detector->GetName() << G4endl;
  G4cout << " PrimitiveScorer " << GetName() << G4endl;
  G4cout << " Number of entries " << EvtMap->entries() << G4endl;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_BB10::PS_STARK_BB10(G4String name, G4int Level, G4double geoX, G4double geoY, G4int nStrip, G4int depth)
    : G4VPrimitiveScorer(name, depth), HCID(-1) {
  m_geoX = geoX;
  m_geoY = geoY;
  m_nStrip = nStrip;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_BB10::~PS_STARK_BB10() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4bool PS_STARK_BB10::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  // contain E_DW, E_UP, E_Bot, Time, DetNbr, and StripWidth
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_BB10::Initialize(G4HCofThisEvent* HCE) {
  EvtMap = new NPS::HitsMap<G4double*>(GetMultiFunctionalDetector()->GetName(), GetName());
  if (HCID < 0)
    HCID = GetCollectionID(0);
  HCE->AddHitsCollection(HCID, (G4VHitsCollection*)EvtMap);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_BB10::EndOfEvent(G4HCofThisEvent*) {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_BB10::clear() {
  std::map<G4int, G4double**>::iterator MapIterator;
  for (MapIterator = EvtMap->GetMap()->begin(); MapIterator != EvtMap->GetMap()->end(); MapIterator++) {
    delete *(MapIterator->second);
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_BB10::DrawAll() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_BB10::PrintAll() {
  G4cout << " MultiFunctionalDet  " << detector->GetName() << G4endl;
  G4cout << " PrimitiveScorer " << GetName() << G4endl;
  G4cout << " Number of entries " << EvtMap->entries() << G4endl;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_QQQ5::PS_STARK_QQQ5(G4String name, G4int Level, G4double inR, G4double outR, G4double phi0, G4double phi1,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
PS_STARK_QQQ5::~PS_STARK_QQQ5() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4bool PS_STARK_QQQ5::ProcessHits(G4Step* aStep, G4TouchableHistory*) {
  // contain E_DW, E_UP, E_Bot, Time, DetNbr, and StripWidth
  ...
  ////////////////////////////////////////////////////////////
  // Not active area: wrong strip numbers -> not hit
  if (m_radialStripNumber < 1 || m_radialStripNumber > m_NRStrip || m_annularStripNumber < 1 ||
      m_annularStripNumber > m_NAStrip)
    return false;
  ////////////////////////////////////////////////////////////
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_QQQ5::Initialize(G4HCofThisEvent* HCE) {
  EvtMap = new NPS::HitsMap<G4double*>(GetMultiFunctionalDetector()->GetName(), GetName());
  if (HCID < 0)
    HCID = GetCollectionID(0);
  HCE->AddHitsCollection(HCID, (G4VHitsCollection*)EvtMap);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_QQQ5::EndOfEvent(G4HCofThisEvent*) {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_QQQ5::clear() {
  std::map<G4int, G4double**>::iterator MapIterator;
  for (MapIterator = EvtMap->GetMap()->begin(); MapIterator != EvtMap->GetMap()->end(); MapIterator++) {
    delete *(MapIterator->second);
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_QQQ5::DrawAll() {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void PS_STARK_QQQ5::PrintAll() {
  G4cout << " MultiFunctionalDet  " << detector->GetName() << G4endl;
  G4cout << " PrimitiveScorer " << GetName() << G4endl;
  G4cout << " Number of entries " << EvtMap->entries() << G4endl;
}
