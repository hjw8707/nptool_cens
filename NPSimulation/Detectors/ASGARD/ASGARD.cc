...
//  const G4double BezelOuterAngle = atan(TaperMagnitude/TaperDepth)*180./M_PI; // 21.8 degrees from NIM A 540 (2005),
//  348; but 22.5 degrees in NIM A 543 (2005), 431
...
G4LogicalVolume* ASGARD::ConstructSegment(G4int seg) {
    ...
    G4Box* Crystal_Box1 = new G4Box("Crystal_Box1", CrystalWidth * 0.6, CrystalWidth * 0.6,
                                    CrystalLength * 0.6);  // rectangular prism which can house the cylinder
    ...
    G4SubtractionSolid* Crystal_withHole = new G4SubtractionSolid(
        "Crystal_withHole", Crystal_Cylinder, Crystal_Hole, BoxRotation, G4ThreeVector(0, 0, CrystalHoleDepth * 0.5));
    ...
    // 45 - 54 = -9 mm, add depth + 9 mm which is 0.1*CrystalLength; i.e. 31 mm depth front volume = 31+9 = 40 mm shift
    // back
    ...
    if (seg == 0) {  // top left, front
        ...
        Crystal_BottomCut = new G4SubtractionSolid(
            "Crystal_BottomCut", Crystal_withHole, Crystal_Box1, BoxRotationInnerBottomBezel,
            G4ThreeVector(2. * CrystalOuterRadius - SegmentFullWidth - CrystalEdgeRadius +
                              0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.) + CrystalWidth * 0.6,
                          0, -0.5 * CrystalLength));  // in mm
        Crystal_RightCut = new G4SubtractionSolid(
            "Crystal_RightCut", Crystal_BottomCut, Crystal_Box1, BoxRotationInnerRightBezel,
            G4ThreeVector(0,
                          -(2. * CrystalOuterRadius - SegmentFullWidth - CrystalEdgeRadius +
                            0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.)) -
                              CrystalWidth * 0.6,
                          -0.5 * CrystalLength));

        // left bezel (applied on segments 0, 1)
        Crystal_LeftBezelCut =
            new G4SubtractionSolid("Crystal_LeftBezelCut", Crystal_RightCut, Crystal_Box1, BoxRotationOuterLeftBezel,
                                   G4ThreeVector(0,
                                                 (CrystalEdgeRadius - TaperMagnitude +
                                                  0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) +
                                                     CrystalWidth * 0.6,
                                                 -CrystalLength * 0.5));

        // need to slice off non-tapered edge at left
        Crystal_LeftCut = new G4SubtractionSolid("Crystal_LeftCut", Crystal_LeftBezelCut, Crystal_Box1, BoxRotation,
                                                 G4ThreeVector(0, CrystalEdgeRadius + CrystalWidth * 0.6, 0));

        // top bezel (applied on segments 0, 3)
        Crystal_TopBezelCut = new G4SubtractionSolid(
            "Crystal_TopBezelCut", Crystal_LeftCut, Crystal_Box1, BoxRotationOuterTopBezel,
            G4ThreeVector(-(CrystalEdgeRadius - TaperMagnitude +
                            0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) -
                              CrystalWidth * 0.6,
                          0, -CrystalLength * 0.5));

        // need to slice off non-tapered edge at top
        Crystal_TopCut = new G4SubtractionSolid("Crystal_TopCut", Crystal_TopBezelCut, Crystal_Box1, BoxRotation,
                                                G4ThreeVector(-CrystalEdgeRadius - CrystalWidth * 0.6, 0, 0));

        Segment_final =
            new G4SubtractionSolid("Segment_final", Crystal_TopCut, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(0, 0, SegmentDepth + 0.1 * CrystalLength));  // cut back volume

    } else if (seg == 1) {  // bottom left, front
        //    Crystal_TopCut = new
        //    G4SubtractionSolid("Crystal_TopCut",Crystal_withHole,Crystal_Box1,BoxRotation,G4ThreeVector(-CrystalWidth*0.6,0,0));
        //    // in mm Crystal_RightCut = new
        //    G4SubtractionSolid("Crystal_RightCut",Crystal_TopCut,Crystal_Box1,BoxRotation,G4ThreeVector(0,-CrystalWidth*0.6,0));
        Crystal_TopCut = new G4SubtractionSolid(
            "Crystal_TopCut", Crystal_withHole, Crystal_Box1, BoxRotationInnerBottomBezel,
            G4ThreeVector((CrystalOuterRadius - SegmentFullWidth +
                           0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.) - CrystalWidth * 0.6),
                          0, -0.5 * CrystalLength));  // in mm

        Crystal_RightCut = new G4SubtractionSolid(
            "Crystal_RightCut", Crystal_TopCut, Crystal_Box1, BoxRotationInnerRightBezel,
            G4ThreeVector(0,
                          -(2. * CrystalOuterRadius - SegmentFullWidth - CrystalEdgeRadius +
                            0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.)) -
                              CrystalWidth * 0.6,
        ...
        Crystal_BottomCut = new G4SubtractionSolid("Crystal_BottomCut", Crystal_RightCut, Crystal_Box1, BoxRotation,
                                                   G4ThreeVector(CrystalReducedRadius + CrystalWidth * 0.6, 0, 0));

        // left bezel (applied on segments 0, 1, 4, 5)
        Crystal_LeftBezelCut =
            new G4SubtractionSolid("Crystal_LeftBezelCut", Crystal_BottomCut, Crystal_Box1, BoxRotationOuterLeftBezel,
                                   G4ThreeVector(0,
                                                 (CrystalEdgeRadius - TaperMagnitude +
                                                  0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) +
                                                     CrystalWidth * 0.6,
        ...
        Segment_final =
            new G4SubtractionSolid("Segment_final", Crystal_LeftCut, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(0, 0, SegmentDepth + 0.1 * CrystalLength));  // cut back volume
        ...
        Crystal_TopCut = new G4SubtractionSolid(
            ...
            G4ThreeVector((CrystalOuterRadius - SegmentFullWidth +
                           0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.) - CrystalWidth * 0.6),
                          0, -0.5 * CrystalLength));
        Crystal_LeftCut = new G4SubtractionSolid(
            "Crystal_LeftCut", Crystal_TopCut, Crystal_Box1, BoxRotationInnerRightBezel,
            G4ThreeVector(0,
                          -(CrystalOuterRadius - SegmentFullWidth +
                            0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.)) +
                              CrystalWidth * 0.6,
                          -0.5 * CrystalLength));

        Crystal_RightCut = new G4SubtractionSolid("Crystal_RightCut", Crystal_LeftCut, Crystal_Box1, BoxRotation,
                                                  G4ThreeVector(0, -CrystalReducedRadius - CrystalWidth * 0.6, 0));
        Crystal_BottomCut = new G4SubtractionSolid("Crystal_BottomCut", Crystal_RightCut, Crystal_Box1, BoxRotation,
                                                   G4ThreeVector(CrystalReducedRadius + CrystalWidth * 0.6, 0, 0));

        Segment_final =
            new G4SubtractionSolid("Segment_final", Crystal_BottomCut, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(0, 0, SegmentDepth + 0.1 * CrystalLength));  // cut back volume

    } else if (seg == 3) {  // top right, front
        Crystal_LeftCut = new G4SubtractionSolid(
            "Crystal_LeftCut", Crystal_withHole, Crystal_Box1, BoxRotationInnerRightBezel,
            G4ThreeVector(0,
                          -(CrystalOuterRadius - SegmentFullWidth +
                            0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.)) +
                              CrystalWidth * 0.6,
                          -0.5 * CrystalLength));
        Crystal_BottomCut = new G4SubtractionSolid(
            "Crystal_BottomCut", Crystal_LeftCut, Crystal_Box1, BoxRotationInnerBottomBezel,
            G4ThreeVector(2. * CrystalOuterRadius - SegmentFullWidth - CrystalEdgeRadius +
                              0.6 * CrystalWidth * (1. / cos(BezelInnerAngle * M_PI / 180.) - 1.) + CrystalWidth * 0.6,
                          0, -0.5 * CrystalLength));  // in mm
        Crystal_RightCut = new G4SubtractionSolid("Crystal_RightCut", Crystal_BottomCut, Crystal_Box1, BoxRotation,
                                                  G4ThreeVector(0, -CrystalReducedRadius - CrystalWidth * 0.6, 0));

        Crystal_TopBezelCut = new G4SubtractionSolid(
            "Crystal_TopBezelCut", Crystal_RightCut, Crystal_Box1, BoxRotationOuterTopBezel,
            G4ThreeVector(-(CrystalEdgeRadius - TaperMagnitude +
                            0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) -
                              CrystalWidth * 0.6,
                          0, -CrystalLength * 0.5));
        ...
        Crystal_BottomCut = new G4SubtractionSolid(
            "Crystal_BottomCut", Crystal_withHole, Crystal_Box1, BoxRotation,
            G4ThreeVector(CrystalWidth * 0.6 - (SegmentFullWidth - CrystalReducedRadius), 0, 0));  // in mm
        Crystal_RightCut = new G4SubtractionSolid(
            "Crystal_RightCut", Crystal_BottomCut, Crystal_Box1, BoxRotation,
            G4ThreeVector(0, -CrystalWidth * 0.6 + (SegmentFullWidth - CrystalReducedRadius), 0));
        Crystal_LeftCut = new G4SubtractionSolid("Crystal_LeftCut", Crystal_RightCut, Crystal_Box1, BoxRotation,
                                                 G4ThreeVector(0, CrystalEdgeRadius + CrystalWidth * 0.6, 0));
        Crystal_TopCut = new G4SubtractionSolid("Crystal_TopCut", Crystal_LeftCut, Crystal_Box1, BoxRotation,
                                                G4ThreeVector(-CrystalEdgeRadius - CrystalWidth * 0.6, 0, 0));

        Segment_final = new G4SubtractionSolid(
            "Segment_final", Crystal_TopCut, Crystal_Box1, BoxRotation,
            G4ThreeVector(0, 0, -(CrystalLength - SegmentDepth) - 0.1 * CrystalLength));  // cut front volume

    } else if (seg == 5) {  // bottom left, back
        Crystal_BottomCut =
            new G4SubtractionSolid("Crystal_BottomCut", Crystal_withHole, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(CrystalWidth * 0.6 + CrystalReducedRadius, 0, 0));  // in mm
        Crystal_RightCut = new G4SubtractionSolid(
            "Crystal_RightCut", Crystal_BottomCut, Crystal_Box1, BoxRotation,
            G4ThreeVector(0, -CrystalWidth * 0.6 + (SegmentFullWidth - CrystalReducedRadius), 0));
        Crystal_LeftCut = new G4SubtractionSolid("Crystal_LeftCut", Crystal_RightCut, Crystal_Box1, BoxRotation,
                                                 G4ThreeVector(0, CrystalEdgeRadius + CrystalWidth * 0.6, 0));
        Crystal_TopCut = new G4SubtractionSolid(
            "Crystal_TopCut", Crystal_LeftCut, Crystal_Box1, BoxRotation,
            G4ThreeVector(-(SegmentFullWidth - CrystalReducedRadius) - CrystalWidth * 0.6, 0, 0));
        ...
        Crystal_BottomCut =
            new G4SubtractionSolid("Crystal_BottomCut", Crystal_withHole, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(CrystalWidth * 0.6 + CrystalReducedRadius, 0, 0));  // in mm
        Crystal_RightCut = new G4SubtractionSolid("Crystal_RightCut", Crystal_BottomCut, Crystal_Box1, BoxRotation,
                                                  G4ThreeVector(0, -CrystalWidth * 0.6 - CrystalReducedRadius, 0));
        Crystal_LeftCut =
            new G4SubtractionSolid("Crystal_LeftCut", Crystal_RightCut, Crystal_Box1, BoxRotation,
                                   G4ThreeVector(0, (SegmentFullWidth - CrystalReducedRadius) + CrystalWidth * 0.6, 0));
        Crystal_TopCut = new G4SubtractionSolid(
            "Crystal_TopCut", Crystal_LeftCut, Crystal_Box1, BoxRotation,
            G4ThreeVector(-(SegmentFullWidth - CrystalReducedRadius) - CrystalWidth * 0.6, 0, 0));
        ...
        logicSegment = new G4LogicalVolume(Crystal_LeftBezelCut, m_MaterialGe, "LogicCrystal", 0, 0, 0);
    }
    logicSegment = new G4LogicalVolume(Segment_final, m_MaterialGe, Form("LogicSegment%d", seg), 0, 0, 0);
...
G4LogicalVolume* ASGARD::ConstructCrystal() {
    ...
    G4Tubs* Crystal_Hole =
        new G4Tubs("Crystal_Hole", 0, CrystalInnerRadius, (CrystalLength - CrystalHoleDepth) * 0.5, 0, 2 * M_PI);
    G4SubtractionSolid* Crystal_withHole =
        new G4SubtractionSolid("Crystal_withHole", Crystal_Cylinder, Crystal_Hole, BoxRotation,
                               G4ThreeVector(-(SegmentFullWidth - CrystalReducedRadius),
                                             (SegmentFullWidth - CrystalReducedRadius), CrystalHoleDepth * 0.5));
    ...
    G4SubtractionSolid* Crystal_BottomCut =
        new G4SubtractionSolid("Crystal_BottomCut", Crystal_withHole, Crystal_Box1, BoxRotation,
                               G4ThreeVector(CrystalReducedRadius + CrystalWidth * 0.6, 0, 0));  // in mm
    G4SubtractionSolid* Crystal_LeftCut =
        new G4SubtractionSolid("Crystal_LeftCut", Crystal_BottomCut, Crystal_Box1, BoxRotation,
                               G4ThreeVector(-CrystalEdgeRadius - CrystalWidth * 0.6, 0, 0));
    G4SubtractionSolid* Crystal_TopCut =
        new G4SubtractionSolid("Crystal_TopCut", Crystal_LeftCut, Crystal_Box1, BoxRotation,
                               G4ThreeVector(0, CrystalEdgeRadius + CrystalWidth * 0.6, 0));
    G4SubtractionSolid* Crystal_RightCut =
        new G4SubtractionSolid("Crystal_RightCut", Crystal_TopCut, Crystal_Box1, BoxRotation,
                               G4ThreeVector(0, -CrystalReducedRadius - CrystalWidth * 0.6, 0));
    ...
    // top bezel (applied on segments 0, 3)
    G4SubtractionSolid* Crystal_TopBezelCut = new G4SubtractionSolid(
        "Crystal_TopBezelCut", Crystal_RightCut, Crystal_Box1, BoxRotationTopBezel,
        G4ThreeVector(
            0,
            (CrystalEdgeRadius - TaperMagnitude + 0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) +
                CrystalWidth * 0.6,
            -CrystalLength * 0.5));
    ...
    // left bezel (applied on segments 0, 1)
    G4SubtractionSolid* Crystal_LeftBezelCut =
        new G4SubtractionSolid("Crystal_LeftBezelCut", Crystal_TopBezelCut, Crystal_Box1, BoxRotationLeftBezel,
                               G4ThreeVector(-(CrystalEdgeRadius - TaperMagnitude +
                                               0.6 * CrystalWidth * (1. / cos(BezelOuterAngle * M_PI / 180.) - 1.)) -
                                                 CrystalWidth * 0.6,
                                             0, -CrystalLength * 0.5));

    G4LogicalVolume* logicCrystal = new G4LogicalVolume(Crystal_LeftBezelCut, m_MaterialGe, "LogicCrystal", 0, 0, 0);
...
G4LogicalVolume* ASGARD::ConstructDewar() {
    ...
    new G4PVPlacement(G4Transform3D(*(new G4RotationMatrix()), G4ThreeVector(0, 0, 0)), LogicN2, "N2 Deware",
                      LogicDewar, false, 1);

    // new G4PVPlacement(G4Transform3D(*(new G4RotationMatrix()), G4ThreeVector(0,0,-90*mm)),
    // LogicN2CF,"N2 Deware",LogicDewar,false,1);
    new G4PVPlacement(G4Transform3D(*(new G4RotationMatrix()), G4ThreeVector(0, 0, -120 * mm)), LogicN2CF, "N2 Deware",
                      LogicDewar, false, 1);
...
void ASGARD::ConstructClover() {
    if (m_LogicClover == 0) {
        ...
        for (int i = 0; i < 8; i++) {
            new G4PVPlacement(G4Transform3D(*CrystalRotationB, *SegmentPosition), logicSegment[i],
                              Form("LogicSegment%d", i), logicCrystal, false,
                              i + 1);  // start from 1 for indexing (0 will be reserved for core)
        }
        ...
        new G4PVPlacement(G4Transform3D(*CrystalRotationB, CrystalPositionB), logicCrystal, "LogicCrystalB",
                          m_LogicClover, true, 0);
        ...
        new G4PVPlacement(G4Transform3D(*CrystalRotationG, CrystalPositionG), logicCrystal, "LogicCrystalG",
                          m_LogicClover, true, 1);
        ...
        new G4PVPlacement(G4Transform3D(*CrystalRotationR, CrystalPositionR), logicCrystal, "LogicCrystalR",
                          m_LogicClover, true, 2);
        ...
        new G4PVPlacement(G4Transform3D(*CrystalRotationW, CrystalPositionW), logicCrystal, "LogicCrystalW",
                          m_LogicClover, true, 3);
    }
}
...
void ASGARD::ConstructDetector(G4LogicalVolume* world) {
    ...
    for (unsigned int i = 0; i < m_CloverId.size(); i++) {
        ...
        // // new G4PVPlacement(G4Transform3D(*DetectorRotation,
        // DetectorPosition+W*((90*mm+(145)*mm)+CapsuleLength*0.5+90*0.5*mm)),
        // //     LogicDewar,"Dewar",world,false,m_CloverId[i]);
        // new G4PVPlacement(G4Transform3D(*DetectorRotation,
        // DetectorPosition+W*((90*mm+(145)*mm)+CapsuleLength*0.5+20*0.5*mm)),
        // 		      LogicDewar,"Dewar",world,false,m_CloverId[i]);
...
void ASGARD::ReadSensitive(const G4Event* event) {
    ...
    for (HPGE_itr = HPGEHitMap->GetMap()->begin(); HPGE_itr != HPGEHitMap->GetMap()->end(); HPGE_itr++) {
        ...
        G4double Energy_reso =
            (1.5 + energy * 0.0015) / 2.355;  // sigma: convert FWHM of (1.5 keV + E(keV)*0.15%) by division of 2.355
...
////////////////////////////////////////////////////////////////////////////////
NPS::VDetector* ASGARD::Construct() { return (NPS::VDetector*)new ASGARD(); }
