...
G4AssemblyVolume* VOICE::BuildAnodeDetector(double radi, double pitch) {
    if (!m_Anode) {
        ...
        G4Box* Anodelayer = new G4Box("VOICE_Anodelayer", VOICE_NS::ElectrodePCBX * 0.5, VOICE_NS::ElectrodePCBY * 0.5,
                                      VOICE_NS::ElectrodePCBZ * 0.5);
        G4Box* AnodeSub = new G4Box("VOICE_AnodeSub", VOICE_NS::ElectrodeSubX * 0.5, VOICE_NS::ElectrodeSubY * 0.5,
                                    VOICE_NS::ElectrodeSubZ * 0.5);
...
G4AssemblyVolume* VOICE::BuildCathodeDetector(double radi, double pitch) {
    if (!m_Cathode) {
        ...
        G4Box* Cathodelayer = new G4Box("VOICE_Cathodelayer", VOICE_NS::ElectrodePCBX * 0.5,
                                        VOICE_NS::ElectrodePCBY * 0.5, VOICE_NS::ElectrodePCBZ * 0.5);
        G4Box* CathodeSub = new G4Box("VOICE_CathodeSub", VOICE_NS::ElectrodeSubX * 0.5, VOICE_NS::ElectrodeSubY * 0.5,
                                      VOICE_NS::ElectrodeSubZ * 0.5);
        ...
        G4LogicalVolume* CathodePCBLogic =
            new G4LogicalVolume(CathodeSolidPCB, PCBMaterial, "CathodePCBLogic", 0, 0, 0);
...
G4AssemblyVolume* VOICE::BuildFGDetector(double radi, double pitch) {
    if (!m_FG) {
        ...
        G4Box* FGlayer = new G4Box("VOICE_FGlayer", VOICE_NS::ElectrodePCBX * 0.5, VOICE_NS::ElectrodePCBY * 0.5,
                                   VOICE_NS::ElectrodePCBZ * 0.5);
        G4Box* FGSub = new G4Box("VOICE_FGSub", VOICE_NS::ElectrodeSubX * 0.5, VOICE_NS::ElectrodeSubY * 0.5,
                                 VOICE_NS::ElectrodeSubZ * 0.5);
...
G4AssemblyVolume* VOICE::BuildEndDetector() {
    if (!m_End) {
        G4Box* enddet = new G4Box("VOICE_End", VOICE_NS::ElectrodePCBX * 0.5, VOICE_NS::ElectrodePCBY * 0.5,
                                  VOICE_NS::endthick * 0.5);
...
void VOICE::ReadConfiguration(NPL::InputParser parser) {
    ...
    for (unsigned int i = 0; i < blocks.size(); i++) {
        ...
        if (blocks[i]->HasTokenList(voice_vec)) {
            if (NPOptionManager::getInstance()->GetVerboseLevel()) cout << endl << "\\\\ VOICE " << i + 1 << endl;
            ...
            if (count_gas_info > 0) {
                cout << "ERROR: Multiple gas info." << endl;
                exit(1);
            }
...
G4Material* VOICE::SetGasType(string gas, double pressure) {
    ...
    if (strcmp(gas.c_str(), "multigas") == 0) {
        if (n_gas < 2 || n_gas > 10) {
            ...
            gasmat = new G4Material("gasmat", (pressure / atmosphere) * (mulgas_density / (g / cm3)) * g / cm3, n_gas,
        ...
        gasmat = new G4Material("gasmat", (pressure / atmosphere) * (filldensity / (g / cm3)) * g / cm3, 2, kStateGas,
                                300. * kelvin, pressure);
    ...
    /*
       ...
       filldensity = 0.00366*g/cm3;
       gasmat=new
    G4Material("gasmat",(pressure/atmosphere)*(filldensity/(g/cm3))*g/cm3,2,kStateGas,300.*kelvin,pressure);
       gasmat->AddElement(C,1);
       gasmat->AddElement(F,4);
       }else if(strcmp(gas.c_str(),"P10")==0){
       filldensity = 0.00156*g/cm3;
    ...
    G4Material("gasmat",(pressure/atmosphere)*(filldensity/(g/cm3))*g/cm3,2,kStateGas,300.*kelvin,pressure);
       ...
       gasmat=new
    G4Material("gasmat",(pressure/atmosphere)*(filldensity/(g/cm3))*g/cm3,1,kStateGas,300.*kelvin,pressure);
    ...
    cout << gasmat->GetChemicalFormula() << "\t" << gasmat->GetPressure() / atmosphere << "\t"
         << gasmat->GetDensity() / (g / cm3) << " is the Gas." << endl;
    return gasmat;
}
...
void VOICE::ConstructDetector(G4LogicalVolume* world) {
    ...
    // edit by minju
    G4Box* GasBox = new G4Box("VOICE_GasBox", VOICE_NS::ElectrodePCBX * 0.5, VOICE_NS::ElectrodePCBY * 0.5,
                              //                          total_length * 0.5 + VOICE_NS::GasDeadLayer);
                              total_length * 0.5);
    ...
    for (unsigned short i = 0; i < m_Type.size(); i++) {
        ...
        if (m_Type[i] == "Anode") {
            ...
            det = BuildCathodeDetector(m_radi[i], m_pitch[i]);
            cout << "========== CATHODE IS FABRICATED ===========" << endl;
        } else if (m_Type[i] == "END") {
            det = BuildEndDetector();
...
void VOICE::ReadSensitive(const G4Event* event) {
    ...
    if (HCID_Anode == -1) HCID_Anode = G4SDManager::GetSDMpointer()->GetCollectionID("AnodeDet/AnodeScorer");
    if (HCID_Cathode == -1) HCID_Cathode = G4SDManager::GetSDMpointer()->GetCollectionID("CathodeDet/CathodeScorer");
    if (HCID_FG == -1) HCID_FG = G4SDManager::GetSDMpointer()->GetCollectionID("FGDet/FGScorer");
    if (HCID_END == -1) HCID_END = G4SDManager::GetSDMpointer()->GetCollectionID("ENDDet/ENDScorer");
    if (HCID_Gas == -1) HCID_Gas = G4SDManager::GetSDMpointer()->GetCollectionID("GasDet/GasScorer");
...
void VOICE::InitializeScorers() {
    ...
    if (!already_exist_A) {
        G4VPrimitiveScorer* AnodeScorer =
            new VOICEScorers::PS_Electrode("AnodeScorer", 0, ElectrodeSubX, ElectrodeSubY, ElectrodePCBZ, 0);
        m_AnodeDetector->RegisterPrimitive(AnodeScorer);
        G4SDManager::GetSDMpointer()->AddNewDetector(m_AnodeDetector);
    }

    if (!already_exist_C) {
        G4VPrimitiveScorer* CathodeScorer =
            new VOICEScorers::PS_Electrode("CathodeScorer", 0, ElectrodeSubX, ElectrodeSubY, ElectrodePCBZ, 0);
        m_CathodeDetector->RegisterPrimitive(CathodeScorer);
        G4SDManager::GetSDMpointer()->AddNewDetector(m_CathodeDetector);
    }

    if (!already_exist_F) {
        G4VPrimitiveScorer* FGScorer =
            new VOICEScorers::PS_Electrode("FGScorer", 0, ElectrodeSubX, ElectrodeSubY, ElectrodePCBZ, 0);
...
////////////////////////////////////////////////////////////////////////////////
NPS::VDetector* VOICE::Construct() { return (NPS::VDetector*)new VOICE(); }
