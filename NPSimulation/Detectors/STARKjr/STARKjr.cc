...
void STARKjr::ReadConfiguration(NPL::InputParser parser) {
    ...
    for (unsigned int i = 0; i < blocks.size(); i++) {
        ...
        if (blocks[i]->HasTokenList(cart)) {
            if (NPOptionManager::getInstance()->GetVerboseLevel()) cout << endl << "////  STARKjr " << i + 1 << endl;
...
NPS::VDetector* STARKjr::Construct() { return (NPS::VDetector*)new STARKjr(); }
