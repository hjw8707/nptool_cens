...
MagField::MagField() : IsLoaded(false), fMagAngle(0), fMagFieldFile(""), fFieldFactor(1.0) {}
////////////////////////////////////////////////////////////////////////
...
void MagField::LoadMagneticField(TString filename) {
    ...
    if (magfile.EndsWith("bin")) {  // check if binary is created
        ...
        std::ifstream magfilebin(Form("%s", magbinfile.Data()), std::ios::in | std::ios::binary);
        if (magfilebin.is_open()) {
            ...
            std::cout << "\x1b[31m"
                      << "fail to get magnetic field data from: " << magfile.Data() << "\x1b[0m" << std::endl;
            magfilebin.close();
            return;
        }
        ...
        while (magfin >> v[0] >> v[1] >> v[2] >> v[3] >> v[4] >> v[5] >> v[6]) {
            ...
            Bx[(int)v[0] % 45][(int)(v[0] / 45)][(int)v[0] % 45] =
                fabs(v[4]) > 0.0001 ? v[4] * kilogauss : 0;  // 45 number of points
            ...
            std::cout << Bx[(int)v[0] % 45][(int)(v[0] / 45)][(int)v[0] % 45] / kilogauss << " "
                      << By[(int)v[0] % 45][(int)(v[0] / 45)][(int)v[0] % 45] / kilogauss << " "
                      << Bz[(int)v[0] % 45][(int)(v[0] / 45)][(int)v[0] % 45] / kilogauss << std::endl;
        }
        magfin.close();
        ...
        std::cout << "\x1b[31m"
                  << "can not identify file type: " << magfile.Data() << "\x1b[0m" << std::endl;
    }
...
void MagField::GetFieldValue(const G4double Pos_lab[4], G4double *B_lab) const {
    ...
    G4double Pos_map[3] = {Pos_lab[0], Pos_lab[1],
                           Pos_lab[2] - (1500 - 121.703) * mm};  // mm 121.793mm is the distance between
    ...
    G4double Pos[3] = {
        -1030 * mm - Pos_lab[0], Pos_lab[1],
        Pos_map[2] -
            121.703 *
                mm};  //-1003mm is the x pos of the center of curvature of sweeper
                      //-1003m-Pos_map[0] translates x=0 of mapper to x=1003 with respect to center of curvature of the
                      //sweeper Pos_map[2]-121.703 translates z=0 of mapper to z=0 of center of curvature of the sweeper
    ...
    std::cout << "FIELD: " << n[0] << " " << Bx[n[0]][n[1]][n[2]] / kilogauss << " " << n[1] << " "
              << By[n[0]][n[1]][n[2]] / kilogauss << " " << n[2] << " " << Bz[n[0]][n[1]][n[2]] / kilogauss << " "
              << Pos_map[2] << " " << std::endl;

    B[0] = mdsq[0] * mdsq[1] * mdsq[2] * Bx[n[0]][n[1]][n[2]] + dsqt[0] * mdsq[1] * mdsq[2] * Bx[n[0] + 1][n[1]][n[2]] +
    ...
    B[1] = mdsq[0] * mdsq[1] * mdsq[2] * By[n[0]][n[1]][n[2]] + dsqt[0] * mdsq[1] * mdsq[2] * By[n[0] + 1][n[1]][n[2]] +
    ...
    B[2] = mdsq[0] * mdsq[1] * mdsq[2] * Bz[n[0]][n[1]][n[2]] + dsqt[0] * mdsq[1] * mdsq[2] * Bz[n[0] + 1][n[1]][n[2]] +
