...
void Fatima::ConstructDetector(G4LogicalVolume* world) {
    /*
      ...
      G4SubtractionSolid* solidChamber=new G4SubtractionSolid("solidChamber",solidExtChamber, solidIntChamber, 0,
    ...
    */
    ...
    for (unsigned int i = 0; i < mysize; i++) {
        ...
        // new G4PVPlacement(0, G4ThreeVector(0., -1.92325*cm/5.5, 0.), logicChamber, "solidChamber",world , false,
        // i+1);
    }
}
...
G4LogicalVolume* Fatima::ConstructDetector() {
    if (!m_LogicalDetector) {
        ...
        G4VisAttributes* invisibleAtt = new G4VisAttributes();
        invisibleAtt->SetVisibility(false);
        m_LogicalDetector->SetVisAttributes(invisibleAtt);
        ...
        new G4PVPlacement(0, G4ThreeVector(0., -1.92325*cm/5.5, 0.), logicChamber, "chamber",m_LogicalDetector , false,
        0);
        ...
        G4Tubs* solidLaBr3Can = new G4Tubs("solidLaBr3Can", 0.5 * CanInnerDiameter, 0.5 * CanOuterDiameter,
                                           0.5 * CanLength, 0. * deg, 360. * deg);
        ...
        G4Tubs* solidLaBr3Win = new G4Tubs("solidLaBr3Win", 0.5 * WinInnerDiameter, 0.5 * WinOuterDiameter,
                                           0.5 * WinLength, 0. * deg, 360. * deg);
        ...
        // G4Tubs* solidLeadA = new G4Tubs("solidLead", 0.5*LeadAMinR, 0.5*LeadAMaxR, 0.5*LeadALength, 0.*deg,
        // 360.*deg); G4LogicalVolume* logicLeadAShield = new G4LogicalVolume(solidLeadA, Lead, "logicLeadAShield", 0,
        // 0, 0);
        ...
        // 360.*deg); G4LogicalVolume* logicLeadBShield = new G4LogicalVolume(solidLeadB, Lead, "logicLeadBShield", 0,
        // 0, 0);
        ...
        // Visualisation of PMT Strip
        // G4VisAttributes* LeadVisAtt = new G4VisAttributes(G4Colour(.66, .66, .66)); //previous in Fatima:
        // (G4Colour(1., 1., 0.)); logicLeadAShield->SetVisAttributes(LeadVisAtt);
        ...
        G4Tubs* solidShield =
            new G4Tubs("solidShield", PMTIn * 0.5, PMTOut * 0.5, PMTThickness * 0.5, 0. * deg, 360. * deg);
        ...
        G4Tubs* solidShieldCan = new G4Tubs("solidShieldCan", 0.5 * WinInnerDiameter, 1 * WinOuterDiameter,
                                            0.5 * WinLength, 0. * deg, 360. * deg);
        G4LogicalVolume* logicShieldCan = new G4LogicalVolume(solidShieldCan, Alu, "logicShieldCan", 0, 0, 0);

        new G4PVPlacement(0, positionShieldCan, logicShieldCan, "Fatima_ShieldCan", m_LogicalDetector, false, 0);
        ...
        G4Tubs* solidHolderCan = new G4Tubs("solidHolderCan", 0.8*LeadBMinR, 0.8*LeadBMaxR, 0.8*LeadBLength, 0.*deg,
        360.*deg); G4LogicalVolume* logicHolderCan = new G4LogicalVolume(solidHolderCan, Alu, "logicHolderCan", 0, 0,
        0);
...
void Fatima::ReadSensitive(const G4Event*) {
    ...
    for (unsigned int i = 0; i < size; i++) {
        time_decay_sum += Scorer->GetTime(i);
        ntime_decay++;
    }
    ...
    for (unsigned int i = 0; i < NDET_FATIMA; i++) {
        if (RandFlat::shoot() < DECAY_PROB) {  // we have a 138La decay
            ...
            m_Event->SetFatimaLaBr3E(i, Energy);
            m_Event->SetFatimaLaBr3T(i, Time);
            //      G4cout<<i<<" "<<Energy/keV<<" "<<Time/us<<G4endl;
        }
    }
    ...
    for (unsigned int i = 0; i < size; i++) {
        ...
        double E = Scorer->GetEnergy(i) * 1000;
        double sigma_energy = pow(E, -0.51) * E / 2.355;
        double Energy = RandGauss::shoot(E, sigma_energy);

        if (Energy > EnergyThreshold) {
            double sigma_time = (Jitter_A / sqrt((Energy + Jitter_B)) + Jitter_C) / 2.355;  // in ns
            //      G4cout<<"sigma_time: "<<sigma_time<<" "<<Energy<<G4endl;
            double Time = RandGauss::shoot(Scorer->GetTime(i), sigma_time) - DIST_FATIMA / c_light;
