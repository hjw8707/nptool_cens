...
G4LogicalVolume* Strasse::BuildInnerDetector() {
  if (!m_InnerDetector) {
    ...
    G4ThreeVector WaferShiftY = G4ThreeVector(
        0, -0.5 * Inner_Wafer_Thickness - Inner_Wafer_AlThickness - 0.5 * Inner_PCB_Thickness - offsetPCB2 - 0.001, 0);
...
G4LogicalVolume* Strasse::BuildOuterDetector() {
  if (!m_OuterDetector) {
    ...
    // Shift along Y to flush the wafer to the pcb ledge on one side
    G4ThreeVector WaferShiftY = G4ThreeVector(
        0, -0.5 * Outer_Wafer_Thickness - Outer_Wafer_AlThickness - 0.5 * Outer_PCB_Thickness - offsetPCB2 - 0.001, 0);
