...
FissionChamber::FissionChamber(){
  ...
  m_VisD2O = new G4VisAttributes(G4Colour(0.,0.5,1.0,1));
  m_VisConcrete = new G4VisAttributes(G4Colour(0.839,0.803,0.803,0.5));
...
void FissionChamber::ReadConfiguration(NPL::InputParser parser){
  ...
  for(unsigned int i = 0 ; i < blocks.size() ; i++){
    if(blocks[i]->HasToken("BuildWall") && blocks[i]->GetInt("BuildWall")==1){
      m_BuildWall=1;
      cout << "BuildWall" << endl;
      string WallMaterial = blocks[i]->GetString("WallMaterial");
      double WallHeight = blocks[i]->GetDouble("WallHeight","mm");
      double WallThickness = blocks[i]->GetDouble("WallThickness","mm");
      double WallLength = blocks[i]->GetDouble("WallLength","mm");
      vector<double> WallPositionX = blocks[i]->GetVectorDouble("WallPositionX","mm");
      vector<double> WallPositionZ = blocks[i]->GetVectorDouble("WallPositionZ","mm");

      m_WallMaterial = WallMaterial;
      m_WallHeight = WallHeight;
      m_WallThickness = WallThickness;
      m_WallLength = WallLength;
      m_WallPositionX = WallPositionX;
      m_WallPositionZ = WallPositionZ;
    }
    if(blocks[i]->HasToken("BuildFloor") && blocks[i]->GetInt("BuildFloor")==1){
      m_BuildFloor=1;
      string FloorMaterial  = blocks[i]->GetString("FloorMaterial");
      TVector3 FloorPos     = blocks[i]->GetTVector3("FloorPos","mm");
      TVector3 FloorWidth   = blocks[i]->GetTVector3("FloorWidth","mm");

      m_FloorMaterial = FloorMaterial;
      m_FloorPos = FloorPos;
      m_FloorWidth = FloorWidth;
    }
    if(blocks[i]->GetInt("BuildNFS")==1){
      cout << "//// Building NFS environment ////" << endl;
      m_BuildNFS = 1;
    }
...
void FissionChamber::ConstructDetector(G4LogicalVolume* world){
  for (unsigned short i = 0 ; i < m_R.size() ; i++) {
    ...
    BuildEnvironment()->MakeImprint(world, Det_pos, Rot, i);
  }

  G4ThreeVector pos = G4ThreeVector(0,0,0);
  G4RotationMatrix* rot = new G4RotationMatrix(0,0,0);
  if(m_BuildNFS) BuildNFS()->MakeImprint(world,pos,rot,0);
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4AssemblyVolume* FissionChamber::BuildNFS(){
  m_NFSRoom = new G4AssemblyVolume();

  G4RotationMatrix *Rv=new G4RotationMatrix(0,0,0);
  G4ThreeVector Tv;
  Tv.setX(0); Tv.setY(0); Tv.setZ(0*m);
  G4Material* heavy_water = MaterialManager::getInstance()->GetMaterialFromLibrary("D2O");
  G4Material* concrete = MaterialManager::getInstance()->GetMaterialFromLibrary("concrete");

  // D2O
  G4Tubs* D2O_solid = new G4Tubs("D2O_cylinder",0,25*cm,15*cm,0,360*deg);
  G4LogicalVolume* D2O_vol = new G4LogicalVolume(D2O_solid, heavy_water,"Heavywater",0,0,0);
  D2O_vol->SetVisAttributes(m_VisD2O); 

  // Concrete
  G4Tubs* concrete_solid = new G4Tubs("concrete_tube",10.1*cm,20*cm,200*cm,0,360*deg);
  G4LogicalVolume* concrete_vol = new G4LogicalVolume(concrete_solid, concrete,"Concrete",0,0,0);
  concrete_vol->SetVisAttributes(m_VisConcrete); 


  //m_NFSRoom->AddPlacedVolume(concrete_vol, Tv, Rv);
  m_NFSRoom->AddPlacedVolume(D2O_vol, Tv, Rv);

  return m_NFSRoom;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4AssemblyVolume* FissionChamber::BuildEnvironment(){
  m_ExperimentalRoomVolume = new G4AssemblyVolume();
  
  G4RotationMatrix *Rv=new G4RotationMatrix(0,0,0);
  G4ThreeVector Tv;
  Tv.setX(0); Tv.setY(0); Tv.setZ(0);

  if(m_BuildFloor==true){

    G4Material* Floor_material = MaterialManager::getInstance()->GetMaterialFromLibrary(m_FloorMaterial);
  
    G4Box*           Floor_solid = new G4Box("Floor", 0.5*m_FloorWidth.X(), 0.5*m_FloorWidth.Y(), 0.5*m_FloorWidth.Z());
    G4LogicalVolume*   Floor_vol = new G4LogicalVolume(Floor_solid, Floor_material, "Floor_logic",0,0,0);

    Floor_vol->SetVisAttributes(m_VisConcrete);
    Tv.setX(m_FloorPos.X());  Tv.setY(m_FloorPos.Y()-0.5*m_FloorWidth.Y());  Tv.setZ(m_FloorPos.Z());

    m_ExperimentalRoomVolume->AddPlacedVolume(Floor_vol, Tv, Rv);
  }
  
  if(m_BuildWall==true){
    
    G4Material*  Wall_material = MaterialManager::getInstance()->GetMaterialFromLibrary(m_FloorMaterial);
  
    G4Box*          Wall_solid; 
    G4LogicalVolume*  Wall_vol; 

    for(int i=0;i<m_WallPositionX.size();i++)
      {
	double WallPosition = m_WallPositionX.at(i);
	double         Sign = 0;
	if(WallPosition!=0) Sign = WallPosition/fabs(WallPosition);
	
	Wall_solid = new G4Box(Form("Wall_X%d",i), 0.5*m_WallThickness, 0.5*m_WallHeight, 0.5*m_WallLength);
	Wall_vol   = new G4LogicalVolume(Wall_solid, Wall_material, Form("Wall_logicX%d",i),0,0,0);
      
	Wall_vol->SetVisAttributes(m_VisConcrete);
	Tv.setX(WallPosition + Sign*0.5*m_WallThickness);  Tv.setY(m_WallHeight/2. + m_FloorPos.Y());  Tv.setZ(0);

	m_ExperimentalRoomVolume->AddPlacedVolume(Wall_vol, Tv, Rv);
      }
    
    for(int i=0;i<m_WallPositionZ.size();i++)
      {
	double WallPosition = m_WallPositionZ.at(i);
	double         Sign = 0;
	if(WallPosition!=0) Sign = WallPosition/fabs(WallPosition);
	
	Wall_solid = new G4Box(Form("Wall_Z%d",i), 0.5*m_WallLength, 0.5*m_WallHeight, 0.5*m_WallThickness);
	Wall_vol   = new G4LogicalVolume(Wall_solid, Wall_material, Form("Wall_logicZ%d",i),0,0,0);
      
	Wall_vol->SetVisAttributes(m_VisConcrete);
	Tv.setX(0);  Tv.setY(m_WallHeight/2. + m_FloorPos.Y());  Tv.setZ(WallPosition + Sign*0.5*m_WallThickness);
  ...
  return m_ExperimentalRoomVolume;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
