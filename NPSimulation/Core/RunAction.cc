...
RunAction::RunAction() : G4UserRunAction() { RootOutput::getInstance()->GetTree()->Branch("Run", &m_RunNumber); }
...
void RunAction::EndOfRunAction(const G4Run* aRun) {
  ...
  // Write DEDX Tables
  std::set<std::string> simulated_particles =
      static_cast<const TrackingAction*>(G4RunManager::GetRunManager()->GetUserTrackingAction())
          ->GetSimulatedParticles();

  MaterialManager::getInstance()->WriteDEDXTable(simulated_particles, 0, 10 * GeV);
  MaterialManager::getInstance()->WriteCrossSectionTable(simulated_particles, 0, 19.9 * MeV);
}
