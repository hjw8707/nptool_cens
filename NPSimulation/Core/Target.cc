...
void Target::ReadConfiguration(NPL::InputParser parser) {
  ...
  if (starget.size() == 1) {
    ...
    if (ctarget[0]->HasTokenList(CoreToken)) {
      ...
      m_TargetMaterial =
          Mat->GetMaterialFromLibrary(ctarget[0]->GetString("Material"), ctarget[0]->GetDouble("Density", "g/cm3"));
...
void Target::ConstructDetector(G4LogicalVolume* world) {
  if (m_TargetType) {  // case of standard target
    ...
    m_CryoTargetSolid = new G4Polycone("solidTarget", 0, 360 * deg, Z.size(), &Z[0], &InnerRadius[0], &OuterRadius[0]);

    m_TargetLogic = new G4LogicalVolume(m_CryoTargetSolid, m_TargetMaterial, "logicTarget");
    ...
    if (m_FrontDeformation != 0) {
      ...
      for (unsigned int i = 0; i < size; i++) {
        ...
        Z.push_back(
            FrontProfile(i * step, m_TargetThickness * 0.5 + m_FrontThickness, m_FrontDeformation, m_FrontRadius));
      }
    }
    ...
    G4LogicalVolume* WindowsLogic = new G4LogicalVolume(WindowsSolid, m_FrontMaterial, "logicWindows");
    ...
    G4LogicalVolume* FrameLogic = new G4LogicalVolume(FrameSolid, m_FrameMaterial, "logicFrame");
    ...
    // Heat Shield
    if (m_HeatShield) {
      G4Tubs* ShieldMain = new G4Tubs("HeatShieldMain", m_ShieldInnerRadius, m_ShieldOuterRadius,
                                      m_ShieldBottomLength + m_ShieldTopLength, 0, 360);

      G4Tubs* ShieldBack = new G4Tubs("HeatShieldBack", 0, m_ShieldBackRadius, m_ShieldInnerRadius, 0, 360);

      G4Tubs* ShieldFront = new G4Tubs("HeatShieldFront", 0, m_ShieldFrontRadius, m_ShieldInnerRadius, 0, 360);
      ...
      G4SubtractionSolid* subtraction = new G4SubtractionSolid("Shield-Back", ShieldMain, ShieldBack, transformBack);
      ...
      G4LogicalVolume* ShieldLogic = new G4LogicalVolume(ShieldSolid, m_ShieldMaterial, "logicShield");

      new G4PVPlacement(&Rot, G4ThreeVector(0, m_ShieldTopLength, 0), ShieldLogic, "ShieldTarget", world, false, 0);
...
void Target::SetReactionRegion() {
  ...
  ecut->SetProductionCut(100 * mm, "e-");
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void Target::RandomGaussian2D(double MeanX, double MeanY, double SigmaX, double SigmaY, double& X, double& Y,
