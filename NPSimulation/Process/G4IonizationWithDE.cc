...
G4IonizationWithDE::G4IonizationWithDE(const G4String& processName, G4ProcessType type)
    : G4VRestDiscreteProcess(processName, type), fTrackSecondariesFirst(false), fStackingFlag(true),
      fNumDriftElectron(1e6), fEmSaturation(nullptr) {
...
// ----------
//
G4VParticleChange* G4IonizationWithDE::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)
...
// -------------
//
G4VParticleChange* G4IonizationWithDE::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
  ...
  G4double pair_energy = aMaterialPropertiesTable->GetConstProperty("DE_PAIRENERGY");
  ...
  for (G4int i = 0; i < tracked_electron; i++) { // always create 100 electron, but with weight
    ...
    G4double Energy = (0.5 * ((electron_mass_c2 / joule) / (c_squared / ((m * m) / (s * s)))) * (v_drift / (m / s)) *
                       (v_drift / (m / s))) *
                      joule;
...
G4double G4IonizationWithDE::GetMeanFreePath(const G4Track&, G4double, G4ForceCondition* condition) {
...
G4double G4IonizationWithDE::GetMeanLifeTime(const G4Track&, G4ForceCondition* condition) {
  *condition = StronglyForced;
