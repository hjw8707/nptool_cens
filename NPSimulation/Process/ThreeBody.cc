...
void NPS::ThreeBody::ReadConfiguration() {
    ...
    if (blocks.size() > 0) {
        ...
        for (unsigned int i = 0; i < blocks.size(); i++) {
            if (blocks[i]->HasToken("Type")) m_type = blocks[i]->GetString("Type");

            if (blocks[i]->HasToken("Beam")) m_Beam = blocks[i]->GetString("Beam");

            if (blocks[i]->HasToken("Target")) m_TargetNucleus = blocks[i]->GetString("Target");
            ...
            if (blocks[i]->HasToken("Light1")) m_Light1 = blocks[i]->GetString("Light1");

            if (blocks[i]->HasToken("Heavy")) m_Heavy = blocks[i]->GetString("Heavy");

            if (blocks[i]->HasToken("Light2")) m_Light2 = blocks[i]->GetString("Light2");

            if (blocks[i]->HasToken("ExcitationEnergyHeavy"))
                m_ExcitationEnergyHeavy = blocks[i]->GetDouble("ExcitationEnergyHeavy", "MeV");

            if (blocks[i]->HasToken("ShootLight1")) m_ShootLight1 = blocks[i]->GetInt("ShootLight1");

            if (blocks[i]->HasToken("ShootLight2")) m_ShootLight2 = blocks[i]->GetInt("ShootLight2");

            if (blocks[i]->HasToken("ShootHeavy")) m_ShootHeavy = blocks[i]->GetInt("ShootHeavy");
            ...
            if (blocks[i]->HasToken("UserEnergyCS")) {
                ...
                if (blocks[i]->HasToken("EnergyCSPath")) m_EnergyCSPath = blocks[i]->GetString("EnergyCSPath");

                if (blocks[i]->HasToken("EnergyCSName")) m_EnergyCSName = blocks[i]->GetString("EnergyCSName");
...
void NPS::ThreeBody::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
    ...
    fastStep.ProposePrimaryTrackFinalKineticEnergyAndDirection(0, pdirection);
    fastStep.ProposePrimaryTrackFinalPosition(worldPosition);
    ...
    double Ecm =
        sqrt(m_N1.Mass() * m_N1.Mass() + m_N2.Mass() * m_N2.Mass() + 2 * m_N2.Mass() * (energy + m_N1.Mass())) -
        m_N1.Mass() - m_N2.Mass();
    if (m_UserEnergyCS == 0) {
    ...
    bool Allowed = false;
    if (Ecm < m_IntermediaryState) Ecm = 0;
    if (m_UserEnergyCS) {
    ...
    if (Allowed) {
        ...
        if (m_IntermediaryStateWidth == 0) {
            ...
            while (true) {
                ...
                if (IntermediaryStateEnergy > SpSn && IntermediaryStateEnergy < Ecm) break;
            }
        }
        ...
        if (m_UserEnergyCS == 0) {
            T1 =
                (pow((Ecm + m_N1.Mass() + m_N2.Mass()), 2.) - (m_N1.Mass() * m_N1.Mass() + m_N2.Mass() * m_N2.Mass())) /
                    (2. * m_N2.Mass()) -
                m_N1.Mass();
        }
        ...
        if (m_type == "sequential") {
            ...
            G4ParticleDefinition* Heavy = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(
