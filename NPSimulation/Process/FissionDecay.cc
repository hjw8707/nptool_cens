...
void FissionDecay::ReadConfiguration() {
    ...
    if (m_FissionDecay.GetFissionToken() > 0) {
        ...
        AttachFissionConditions();
        if (!RootOutput::getInstance()->GetTree()->FindBranch("FissionConditions")) {
            RootOutput::getInstance()->GetTree()->Branch("FissionConditions", "TFissionConditions",
                                                         &m_FissionConditions);
        }
    }
}
...
void FissionDecay::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
    ...
    G4bool IsFissionDecay = m_FissionDecay.GenerateEvent(
        NPL::ChangeNameFromG4Standard(m_CurrentName), m_ExcitationEnergy, energy, pdirection.x(), pdirection.y(),
        pdirection.z(), FissionFragment, Ex, DEK, DPx, DPy, DPz, TKE, KE1, KE2);
