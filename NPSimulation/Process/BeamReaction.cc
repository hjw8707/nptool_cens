...
G4bool NPS::BeamReaction::ModelTrigger(const G4FastTrack& fastTrack) {
  ...
  G4ThreeVector V = fastTrack.GetPrimaryTrackLocalDirection(); //(PrimaryTrack->GetMomentum()).unit();
  ...
  if (is_first && m_shoot) {
    /* Does occur rarely when event is tangent to the target surface and scatters out
       ...
       simultaneously" << std::endl; std::cout << "m_shoot: " << m_shoot << std::endl; std::cout << "rand: " <<
       m_rand
       << std::endl; std::cout << "to_entrance: " << to_entrance << std::endl; std::cout << "to_exit: " << to_exit
       << std::endl; std::cout << "length: " << m_length << std::endl; std::cout << "step: " << m_StepSize <<
       std::endl; std::cout << "Z: " << m_Z << std::endl; std::cout << "S: " << m_S << std::endl;
       */
    m_shoot = false;
  }
  ...
  m_StepSize = PrimaryTrack->GetVolume()->GetLogicalVolume()->GetUserLimits()->GetMaxAllowedStep(
      *PrimaryTrack); // If the condition is met, the event is generated
...
void NPS::BeamReaction::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep) {
  ...
  m_shoot = false;
  m_length = abs(m_length); // m_length is more length from the step to the reaction vertex
  // Get the track info
  const G4Track* PrimaryTrack = fastTrack.GetPrimaryTrack();
  ...
  //////////////////////////////////////////////////////////////
  // Transformation between local and world coordinates //////
  //////////////////////////////////////////////////////////////
  // Local -> World: fastTrack.GetInverseAffineTransformation()->TransformPoint(localPosition)
  // World -> Local: fastTrack.GetAffineTransformation()->TransformPoint(worldPosition)
  //////////////////////////////////////////////////////////////
  ...
  G4ThreeVector ldir = localdir; // pdirection;
  ldir *= m_length;
  localPosition = localPosition + ldir;

  // Set the end of the step conditions
  fastStep.ProposePrimaryTrackFinalKineticEnergyAndDirection(0, localdir); // local direction
  fastStep.ProposePrimaryTrackFinalPosition(localPosition);                // local coordinates
  fastStep.ProposeTotalEnergyDeposited(0);
  fastStep.ProposePrimaryTrackFinalTime(time); // FIXME
  ...
  if (m_ReactionType == TwoBody) {
    ...
    G4ThreeVector newWorldPosition = fastTrack.GetInverseAffineTransformation()->TransformPoint(localPosition);
    // Emitt secondary
    if (m_Reaction.GetShoot3()) {
      ...
      fastStep.CreateSecondaryTrack(particle3, newWorldPosition, time, false);
    }
    ...
    if (m_Reaction.GetShoot4()) {
      ...
      fastStep.CreateSecondaryTrack(particle4, newWorldPosition, time, false);
    }
    ///////////////////////////////////////
